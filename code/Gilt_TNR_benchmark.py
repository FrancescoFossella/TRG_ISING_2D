...
def trg(A1, A2, log_fact, pars, **kwargs):
    ...
    **kwargs: Keyword arguments may be used to override the parameters
    ...
    B1, B2, err = A1.split(
        [0, 1], [2, 3], chis=pars["cg_chis"], eps=pars["cg_eps"], return_rel_err=True
    )
    if verbose:
        chi = type(B1).flatten_dim(B1.shape[2])
        status_print(
            "TRG splitting, done.", "Error = {:.3e}".format(err), "chi = {}".format(chi)
        )
    ...
    C1, C2, err = A2.split(
        [2, 1], [0, 3], chis=pars["cg_chis"], eps=pars["cg_eps"], return_rel_err=True
    )
    if verbose:
    ...
    A = ncon((B2, C2, B1, C1), ([-1, 11, 1], [-2, 11, 2], [10, 2, -3], [10, 1, -4]))
    if verbose:
...
def apply_gilt(A1, A2, pars, where=None):
    ...
    Rp1, s, Rp2, spliterr = Rp.split(
        0, 1, eps=spliteps, return_rel_err=True, return_sings=True
    )
